...
namespace px4_ros2
{
        ...
        _config_overrides(node, topic_namespace_prefix)
  {
    _vehicle_status_sub = _node.create_subscription<px4_msgs::msg::VehicleStatus>(
        topic_namespace_prefix + "fmu/out/vehicle_status" + px4_ros2::getMessageNameVersion<px4_msgs::msg::VehicleStatus>(), rclcpp::QoS(1).best_effort(),
    ...
    opts.callback_group = _ack_callback_group;

    _vehicle_command_ack_sub =
        _node.create_subscription<px4_msgs::msg::VehicleCommandAck>(
            _topic_namespace_prefix + "fmu/out/vehicle_command_ack" + px4_ros2::getMessageNameVersion<px4_msgs::msg::VehicleCommandAck>(),
      ...
      float param5, float param6, float param7)
  {
    ...
    while (_vehicle_command_ack_sub->get_publisher_count() == 0)
    {
    ...
    for (int attempt = 0; attempt < 5 && !got_reply; ++attempt)
    {
      ...
      while (!got_reply)
      {
        ...
        // If we’ve already timed out, break out.
        if (elapsed >= timeout)
        {
          break;
        }
        ...
        RCLCPP_DEBUG(
            _node.get_logger(),
            "Waiting for ACK, remaining = %lld ms", static_cast<long long>(ms));

        auto ret = wait_set.wait(remaining);
        if (ret.kind() != rclcpp::WaitResultKind::Ready)
        {
          RCLCPP_DEBUG(_node.get_logger(), "ACK wait timeout");
          continue;
        }

        px4_msgs::msg::VehicleCommandAck ack;
        rclcpp::MessageInfo info;
        if (_vehicle_command_ack_sub->take(ack, info) && ack.command == cmd.command && ack.target_component == cmd.source_component)
        {
          if (ack.result ==
          ...
          {
            RCLCPP_WARN(_node.get_logger(), "Command NACK: %u", ack.result);
          }
          got_reply = true;
        }
      }
    ...
    if (!got_reply)
    {
      ...
      RCLCPP_WARN(
          _node.get_logger(),
          "Cmd %i: timeout, no ack received", cmd.command);
    }
    ...
    //                 "GET ANOTHER RESULT??");
    //             if (ack.result ==
    //                 px4_msgs::msg::VehicleCommandAck::VEHICLE_CMD_RESULT_FAILED)
    //             {
    //               RCLCPP_WARN(
    //                   _node.get_logger(),
  ...
  void ModeExecutorBase::vehicleStatusUpdated(const px4_msgs::msg::VehicleStatus::UniquePtr &msg)
  {
    ...
    if (_is_in_charge != is_in_charge)
    {
      ...
      {
        callOnDeactivate(
            msg->failsafe ? DeactivateReason::FailsafeActivated : DeactivateReason::Other);
      }
    }
      ...
      const std::string &topic_namespace_prefix)
  {
    _mode_completed_sub = node.create_subscription<px4_msgs::msg::ModeCompleted>(
        topic_namespace_prefix + "fmu/out/mode_completed" + px4_ros2::getMessageNameVersion<px4_msgs::msg::ModeCompleted>(), rclcpp::QoS(1).best_effort(),
